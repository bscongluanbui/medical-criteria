...
function renderSources(){
  $('#sources-list').innerHTML=sources.length?sources.map(s=>`<article class="source-card"><span class="pill">${esc(s.id)}</span><h3>${esc(s.title)}</h3><p class="muted">${esc(s.organization)} · ${esc(s.version)} · ${s.pdf_pages} ${s.source_format==='pdf'||!s.source_format?'trang PDF':'đoạn'}</p><p class="fine-print">SHA256<br><code>${esc(s.sha256)}</code></p>${s.official_url?`<a href="${esc(safeURL(s.official_url))}" target="_blank" rel="noopener noreferrer">Mở nguồn chính thức ↗</a>`:`<small>Nguồn lưu trong thư viện nội bộ</small>`}</article>`).join(''):'<div class="empty"><h3>Chưa có tài liệu nguồn</h3><p>Quản trị viên đăng ký tài liệu gốc để bắt đầu biên soạn.</p></div>';
}
...
async function openCard(row){
  ...
  $('#evidence-list').innerHTML=selected.card.evidence.map(e=>{const s=sources.find(s=>s.id===e.document_version_id);return `<div class="evidence-item"><strong>${esc(s?.title||e.document_version_id||'Chưa chọn nguồn')}</strong><p class="fine-print">${s?.source_format==='pdf'||!s?.source_format?'Trang PDF':'Đoạn văn bản'} ${e.pdf_page} · ${esc(e.section||'Chưa có mục')}</p><blockquote>${esc(e.quote||'Điền trích dẫn nguyên văn trong cấu trúc JSON.')}</blockquote>${s?.official_url?`<a href="${esc(safeURL(s.official_url))}" target="_blank" rel="noopener noreferrer">Mở nguồn gốc ↗</a>`:''}</div>`;}).join('');
...
function verificationText(v){return `Gemini (${v?.gemini_model||'chưa ghi model'}): ${v?.gemini||'NOT_RECORDED'} · ChatGPT: ${v?.chatgpt||'GPT_UNVERIFIED'} · Bác sĩ: ${v?.doctor||'DOCTOR_UNVERIFIED'}`;}
